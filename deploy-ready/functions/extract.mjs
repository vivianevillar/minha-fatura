...
function deduplicar(parsed) {
  // Para cada grupo/portador, remove transações duplicadas
  // Duplicata = mesma data + valor + desc similar, mas parcela diferente (próximas faturas)
  parsed.grupos.forEach(grupo => {
    const vistas = new Map();
    grupo.txs = grupo.txs.filter(tx => {
      // Chave: data + valor arredondado + 4 primeiras palavras da desc
      const descKey = tx.desc.toLowerCase().replace(/\s+/g,' ').split(' ').slice(0,4).join(' ');
      const key = `${tx.data}|${tx.val}|${descKey}`;
      ...
      // Já vimos essa transação — manter a que tem menor número de parcela atual
      const existente = vistas.get(key);
      const parcAtual = tx.parc ? parseInt(tx.parc.split('/')[0]) : 1;
      const parcExist = existente.parc ? parseInt(existente.parc.split('/')[0]) : 1;

      if (parcAtual < parcExist) {
        // Esta é mais antiga/atual — substituir a existente
        grupo.txs[grupo.txs.indexOf(existente)] = tx;
        vistas.set(key, tx);
      }
      return false;
    });
  });
  console.log("Apos dedup:", parsed.grupos.reduce((s,g)=>s+g.txs.length,0), "transacoes");
}
