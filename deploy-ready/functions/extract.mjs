export async function handler(event) {
  ...
  const prompt = `Extraia as transações desta fatura de cartão brasileiro. Retorne SOMENTE o objeto JSON puro, sem markdown, sem blocos de código, sem texto antes ou depois.
...
Regras: portador=nome de pessoa em seção da fatura (nunca CPF/CNPJ). desc=nome limpo sem prefixos. neg=true para estornos. NÃO incluir próximas faturas.`;
  ...
  try {
    ...
    let parsed;
    // Remove markdown fences globalmente (sem âncora ^)
