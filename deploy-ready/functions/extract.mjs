...
- CRITICAL: output MINIFIED JSON on a single line. Zero spaces, zero newlines, zero indentation. Example: {"a":1,"b":[2,3]} NOT { "a": 1 }`;
  ...
  try {
    ...
    // Fix escaped JSON: model sometimes outputs {\"key\":\"val\"} instead of {"key":"val"}
    // Detect by checking if position 1 is a literal backslash (char code 92)
    if (compacted.length > 2 && compacted.charCodeAt(1) === 92) {
      compacted = compacted.replace(/\\"/g, '"').replace(/\\\\/g, '\\');
      console.log("Unescaped backslash-quoted JSON, length:", compacted.length);
    }
